...
- 播放器的诊断信息、`com.tcl.cyberui` 的版本、`com.tcl.usercenter` 的安装判断、待安装广告 APK 的包名保持各自来源。播放器可反射加载 `UniTrackerImpl` 接收诊断信息，但本模块配套的三个 TCL AAR 不包含该可选组件，也没有声明此依赖。
...
身份补丁已随 `v1.0.18` 发布；单包构建已随 `v1.0.19` 发布，远程主 AAR 及依赖元数据已下载核对。没有进行设备实播或真实流量验证，静态路径和本地产物核对不能证明 TCL 服务端已接受登记配置。
