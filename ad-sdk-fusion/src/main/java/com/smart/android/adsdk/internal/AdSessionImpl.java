...
final class AdSessionImpl implements ChannelSession {
    ...
    public void start() {
        synchronized (lock) {
            if (startRequested || terminal || cancellationRequested) {
                return;
            }
            startRequested = true;
    ...
    private void applyAuthorizationResponse(FlowAuthorizedConfig config) {
        ...
        synchronized (lock) {
            if (terminal || cancellationRequested || authorizationResponseReceived) return;
    ...
    private void handleAuthorized(FlowAuthorizedConfig config, long receivedAtMs) {
        ...
        synchronized (lock) {
            if (terminal || cancellationRequested || requestedReported) return;
    ...
    public void pause() {
        ...
        synchronized (lock) {
            if (terminal || cancellationRequested || state != AdState.PLAYING || player == null) {
                return;
            }
    ...
    public void resume() {
        ...
        AdPlayer activePlayer;
        synchronized (lock) {
            if (terminal || cancellationRequested || state != AdState.PAUSED || player == null) {
    ...
    public void requestCancellation() {
        synchronized (lock) {
            ...
            if (player != null) player.requestCancellation();
        }
    }
    ...
    private void handleResolved(RemoteAdConfigResult result) {
        ...
        synchronized (lock) {
            shouldRelease = terminal || cancellationRequested;
            if (!shouldRelease) {
                player = newPlayer;
    ...
    private void notifyLoaded(long receivedAtMs) {
        synchronized (lock) {
            if (terminal || cancellationRequested || loadedNotified) {
                return;
            }
            loadedNotified = true;
    ...
    private void notifyStarted(long receivedAtMs) {
        synchronized (lock) {
            if (terminal || cancellationRequested || startedNotified) {
                return;
            }
    ...
    private void finish(AdResult result) {
        ...
        synchronized (lock) {
            ...
            outcome = cancellationRequested ? AdResult.cancelled() : result;
            terminal = true;
        ...
        trace("finish phase=" + phase + " elapsedMs="
            + Math.max(0L, finishedAtMs - callbackTimeoutStartedAtMs) + " " + outcome);
        ...
        safely("report finished", () -> reporter.finished(requestId, createdAtMs, outcome, diagnostics()));
        safely("report flow trace", () -> flowTrace.finish(reporter, requestId, outcome, diagnostics()));
        dispatcher.dispatch(() -> {
            trace("onFinished dispatch " + outcome);
            listener.onFinished(this, outcome);
