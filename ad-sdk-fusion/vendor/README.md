...
这四份 AAR 仅作为构建输入，代码、资源和 consumer rules 均合入 `ad-sdk-fusion`，不再独立发布 Maven 模块。原有单家 SDK 保持独立。
...
| cmp | adsdk_overseas_cmp_sdk-2.8.02.aar | `5a6b19906daaf715c48a079aa185c908b906d23536a709dc57589f85e07f833f` |

## 融合版身份配置补丁

融合模块的 `patchTclFusionAar` 生成 `build/tcl/base-patched.aar`，再由 `prepareEmbeddedTcl` 与原始 media/player/cmp 一起打包。最终只发布融合主 AAR。
...
这份中间补丁 AAR 不对外发布。最终主 AAR 内含 `libs/tcl-base.jar`、`libs/tcl-media.jar`、`libs/tcl-player.jar`、`libs/tcl-cmp.jar`，以及合并的资源、consumer rules 和身份桥接类。
...
`TclFusionBundleTask` 提取四份输入的资源和混淆规则，并把四份 classes.jar 嵌入主 AAR。TCL 原资源命名空间的字段引用改指融合模块的 R 类，使 Android 宿主能为单个 AAR 生成正确资源 ID；不修改资源名、广告参数和固定字符串。原始 AAR 文件保持不变。原 media/player Manifest 的 `allowBackup=false` 由融合 Manifest 保留，网络权限原已声明。

`settings.gradle` 不再包含三个 TCL 子项目；JitPack 仅执行融合主模块的发布任务。生成的 POM 和 Gradle module metadata 不包含 `ad-sdk-fusion-tcl-*` 依赖。

先前未含 CMP 的三组件单包构建已随 `v1.0.19` 发布。远程主 AAR、POM 和 Gradle module metadata 已下载核对，JitPack 产物列表不再包含三个 TCL 子模块。

`v1.0.19` 的本地验证记录：单包 AAR 构建及 Maven 发布通过；仅使用该依赖的宿主 Release APK（R8 混淆开启）构建通过。主 POM/module metadata 无 TCL 子模块依赖，2,405 个 TCL 类完整内嵌，20 个资源字段存在；只重定向 29 处资源字段引用，其他类内容保持原样，84 项身份断言通过。未进行设备实播。


## CMP 恢复

原厂 CMP 2.8.02 包含 167 个类、99 个资源条目，无 JNI/native 文件和嵌套 JAR；其 Manifest 只要求已有的 `ACCESS_NETWORK_STATE` 权限，无 Activity、Service 或 Provider 声明。它使用的 Fragment、Leanback、RecyclerView、Lifecycle、协程、Glide、Gson 和 IAB TCF 编码器由融合模块的现有依赖提供。

CMP 的 `com.tcl.ff.component.oversea.cmp.R$*` 字段引用在嵌入时统一重定向到融合模块 R 类。检查该原厂包未发现 `Resources.getIdentifier` 或反射查找 R 类的调用。资源名称及供应商固定字符串保持原样。

CMP 请求基类 `com.tcl.ff.component.oversea.model.requset.a` 的 `appbundle`、`appMd5`、`appVersion` 分别经 `BasicParameters.getPackageName/getSignature/getVersionName` 读取，共用已有 `TclIdentityBridge` 配置；请求使用 base 的公共 HTTP 层，`XTCL-App` 继续由同一身份补丁提供。CMP 跳转参数 `CmpJumpBean.getPackageName` 是服务端目标页面包名，保持原样。无需新增 CMP 身份字节码补丁。
