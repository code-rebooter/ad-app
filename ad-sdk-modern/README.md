...
当前源码新增了系统属性日志、`onFinished` 自动输出结束详情和原始错误码回传，使用方法见 [诊断日志说明](DIAGNOSTICS.md)。整轮流程上报、各阶段耗时及授权结果的同步范围见 [通用 SDK 流程同步说明](FLOW_SYNC.md)。

这些改动尚未发布，不包含在上面的远程 `v1.0.15` 中；依赖坐标保持原样，待发布时再更新。
