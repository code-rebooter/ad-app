...
private object HaierLsapFormalAd {
    ...
    fun showAd(
        ...
        Log.i(
            TAG,
            "正式链路：开始请求海尔 LSAP 广告，requestId=$requestId，adId=$adId，tagId=$TAG_ID，hidden=${AdDisplayConfig.isHiddenMode()}，container=${flRoot.width}x${flRoot.height}"
        )
        ...
        Hq008ConsentLogReporter.report(
            ...
            eventMessage = "sdk=haier_lsap,sdkEntry=unified,requestId=$requestId,adId=${adId.orEmpty()},tagId=$TAG_ID,hidden=${AdDisplayConfig.isHiddenMode()},containerWidth=${flRoot.width},containerHeight=${flRoot.height}"
        )
        ...
        currentRequest
            ...
            ?.let { request ->
                ...
                Hq008ConsentLogReporter.report(
                    ...
                    eventMessage = "sdk=haier_lsap,sdkEntry=unified,requestId=${request.requestId},adId=${request.adId.orEmpty()},hidden=${AdDisplayConfig.isHiddenMode()},stage=window_destroy,reason=window_hidden"
                )
            }
        clearTimeout()
    ...
    private fun ensureInitialized(): Boolean {
        return runCatching {
            if (!UnifiedAdSdk.isInitialized()) {
                ...
                Log.i(TAG, "正式链路：已发起海尔统一广告 SDK 初始化，appKey=$APP_KEY")
            }
    ...
    private fun startAd(request: PendingShowRequest) {
        ...
        runCatching {
            ...
            Log.i(TAG, "正式链路：已调用 UnifiedAdSdk.requestAd，requestId=${request.requestId}，tagId=$TAG_ID")
            armTimeout(request)
        }.onFailure { error ->
            failRequest(
                request = request,
    ...
    private fun notifyLoaded(request: PendingShowRequest) {
        ...
        Hq008ConsentLogReporter.report(
            eventType = "AD_LOADED",
            eventMessage = "sdk=haier_lsap,sdkEntry=unified,requestId=${request.requestId},adId=${request.adId.orEmpty()},tagId=$TAG_ID,hidden=${AdDisplayConfig.isHiddenMode()},requestToLoadDurationMs=${request.requestToLoadDurationMs()}"
        )
    }
    ...
    private fun notifyStarted(request: PendingShowRequest, container: ViewGroup) {
        ...
        Hq008ConsentLogReporter.report(
            ...
            eventMessage = "sdk=haier_lsap,sdkEntry=unified,requestId=${request.requestId},adId=${request.adId.orEmpty()},tagId=$TAG_ID,hidden=${AdDisplayConfig.isHiddenMode()},childCount=${container.childCount},requestToStartDurationMs=${request.requestToStartDurationMs()}"
        )
    ...
    private fun completeRequest(request: PendingShowRequest) {
        ...
        Hq008ConsentLogReporter.report(
            ...
            eventMessage = "sdk=haier_lsap,sdkEntry=unified,requestId=${request.requestId},adId=${request.adId.orEmpty()},tagId=$TAG_ID,hidden=${AdDisplayConfig.isHiddenMode()},playbackDurationMs=${request.playbackDurationMs()},requestTotalDurationMs=${request.requestTotalDurationMs()},finishReason=request_finished"
        )
        releaseCurrentSession()
        currentRequest = null
    ...
    private fun failRequest(
        ...
        Hq008ConsentLogReporter.report(
            ...
            eventMessage = "sdk=haier_lsap,sdkEntry=unified,requestId=${request.requestId},adId=${request.adId.orEmpty()},tagId=$TAG_ID,hidden=${AdDisplayConfig.isHiddenMode()},stage=$stage,reason=$reason,error=$errorText"
        )
        if (releaseSession) {
            releaseCurrentSession()
        }
