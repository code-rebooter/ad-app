...
@UnstableApi
final class ImaAdPlayer implements AdPlayer {
    ...
    ImaAdPlayer(Context context, ViewGroup container, Listener listener) {
    ...
    private void createPlayer(int adLoadTimeoutMs) {
        ...
        adsLoader = new ImaAdsLoader.Builder(context)
            ...
            .setAdErrorListener(error -> fail(
                eventGateHasStarted()
                    ...
                    : AdErrorCode.IMA_LOAD_ERROR,
                error == null ? "Unknown IMA error" : error.toString(),
                null
            ))
            .build();
        ...
        DefaultMediaSourceFactory mediaSourceFactory =
            new DefaultMediaSourceFactory(new DefaultDataSource.Factory(context))
                .setLocalAdInsertionComponents(adsConfiguration -> adsLoader, playerView);
        ...
        player.addListener(new Player.Listener() {
            ...
            public void onPlaybackStateChanged(int playbackState) {
                if (playbackState == Player.STATE_ENDED && !eventGateHasStarted()) {
                    fail(
                        AdErrorCode.IMA_LOAD_ERROR,
                        "Ad playback ended before IMA STARTED",
                        null
                    );
                }
            }
    ...
    private AdsMediaSource createAdMediaSource(String adTagUrl) {
        ...
        return new AdsMediaSource(
            ...
            "google_video_ad_" + System.currentTimeMillis(),
        ...
        switch (event.getType()) {
            ...
            case ALL_ADS_COMPLETED:
                if (eventGateHasStarted()) {
                    ...
                    fail(
                        AdErrorCode.IMA_LOAD_ERROR,
                        "IMA completed before the ad started",
                        null
                    );
                }
                break;
            case SKIPPED:
                if (eventGateHasStarted()) {
                    ...
                    fail(
                        AdErrorCode.IMA_LOAD_ERROR,
                        "IMA skipped before the ad started",
                        null
                    );
                }
                break;
            case CONTENT_RESUME_REQUESTED:
                if (!eventGateHasStarted()) {
                    fail(
                        AdErrorCode.IMA_LOAD_ERROR,
                        "IMA resumed content before the ad started",
                        null
                    );
                }
                break;
    ...
    private void armStartupTimeout() {
        ...
        startupTimeoutAction = () -> fail(
            AdErrorCode.TIMEOUT,
            "IMA ad did not start within " + startupTimeoutMs + " ms",
            null
        );
